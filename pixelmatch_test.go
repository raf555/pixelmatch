...
func TestIdenticalImages(t *testing.T) {
	w, h := 32, 32
	img := makeRGBA(w, h, func(x, y int) (uint8, uint8, uint8, uint8) {
	...
	n, err := pixelmatch.Match(img, img2, out, w, h, nil)
	if err != nil {
		t.Fatal(err)
	}
	if n != 0 {
...
func TestIdenticalNoOutput(t *testing.T) {
	...
	img2 := make([]byte, len(img))
	copy(img2, img)
	n, err := pixelmatch.Match(img, img2, nil, w, h, nil)
	if err != nil {
		t.Fatal(err)
	}
	if n != 0 {
...
func TestCompletelyDifferent(t *testing.T) {
	...
	img1 := makeRGBA(w, h, func(x, y int) (uint8, uint8, uint8, uint8) {
		return 0, 0, 0, 255
	})
	img2 := makeRGBA(w, h, func(x, y int) (uint8, uint8, uint8, uint8) {
		return 255, 255, 255, 255
	})
	out := make([]byte, len(img1))
	n, err := pixelmatch.Match(img1, img2, out, w, h, nil)
	if err != nil {
		t.Fatal(err)
	}
	if n != w*h {
...
func TestPartialDifference(t *testing.T) {
	...
	n, err := pixelmatch.Match(img1, img2, nil, w, h, nil)
	if err != nil {
		t.Fatal(err)
	}
	if n != 7 {
...
func TestThresholdSensitivity(t *testing.T) {
	...
	loose := pixelmatch.DefaultOptions()
	loose.Threshold = 0.5
	n, err := pixelmatch.Match(img1, img2, nil, w, h, &loose)
	if err != nil {
		t.Fatal(err)
	}
	if n != 0 {
	...
	tight := pixelmatch.DefaultOptions()
	tight.Threshold = 0.0
	n, err = pixelmatch.Match(img1, img2, nil, w, h, &tight)
	if err != nil {
		t.Fatal(err)
	}
	if n != w*h {
...
func TestDiffColorAlt(t *testing.T) {
	...
	out := make([]byte, len(img1))
	n, err := pixelmatch.Match(img1, img2, out, w, h, &opts)
	if err != nil {
		t.Fatal(err)
	}
	if n != w*h {
...
func TestSizeMismatchErrors(t *testing.T) {
	...
	img2 := make([]byte, 20)
	if _, err := pixelmatch.Match(img1, img2, nil, 2, 2, nil); err == nil {
		t.Error("expected error on mismatched sizes")
	}
	if _, err := pixelmatch.Match(img1, img1, nil, 3, 3, nil); err == nil {
...
func TestAntialiasingDetection(t *testing.T) {
	...
	nAA, err := pixelmatch.Match(img1, img2, nil, w, h, nil)
	if err != nil {
		t.Fatal(err)
	}
	opts := pixelmatch.DefaultOptions()
	opts.IncludeAA = true
	nNoAA, err := pixelmatch.Match(img1, img2, nil, w, h, &opts)
	if err != nil {
		t.Fatal(err)
	}
	if !(nAA < nNoAA) {
